...
const Services = () => {
  ...
  return (
    <section className="min-h-screen h-screen 2xl:h-screen items-center justify-center bg-blue-400 dark:from-gray-950 dark:via-gray-900 dark:to-blue-950 px-4 sm:px-8 md:px-16 lg:px-20 py-12">
      <div className="max-w-7xl mx-auto w-full">
        <div className="text-center">
          <h1
            className="text-4xl md:text-5xl font-extrabold text-gray-900 dark:text-black tracking-tight mt-2 mb-2"
            style={{ padding: 40 }}
          >
            Our Services
          </h1>
          <p
            className="text-xl text-white font-bold dark:text-gray-300 max-w-2xl mx-auto font-serif"
            style={{ paddingTop: 15 }}
          >
            Explore the range of technology solutions we offer to boost your
            business growth and efficiency.
          </p>
        </div>
        <div
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 h-auto"
        ...
        >
          {services.map((service, index) => (
            ...
            >
              <div className="flex flex-col px-4 py-4 items-center flex-1 w-full">
                {service.icon}
                <h2 className="text-2xl font-extrabold text-blue-400  mb-3 mt-1 transition-colors duration-300">
                  {service.title}
                </h2>
                <p className="text-gray-600 max-w-md dark:text-gray-300 leading-relaxed mb-8">
                  {service.description}
                </p>
              </div>
              <button className="bg-blue-400 text-white font-semibold py-2 px-6 rounded-lg shadow hover:bg-blue-700 transition-transform transition-colors duration-200 mt-2 group-hover:scale-105 group-hover:shadow-lg">
                Learn More
              </button>
            </div>
          ))}
